...
public class ChessMatch {
	...
	private ChessPiece enPassantVulnerab;
	...
	public ChessPiece EnPassantVulnerab() {
		return enPassantVulnerab;
	}
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (testCheck(currentPlayer)) {
			undeMove(source, target, capturedPiece);
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (p instanceof King && target.getColum() == source.getColum() + 2) {
			Position sourceR = new Position(source.getRow(), source.getColum() + 3);
			Position targetR = new Position(source.getRow(), source.getColum() + 1);
		...
		// #specialmove castling kingside rook
		if (p instanceof King && target.getColum() == source.getColum() - 2) {
			Position sourceR = new Position(source.getRow(), source.getColum() - 4);
			Position targetR = new Position(source.getRow(), source.getColum() - 1);
			ChessPiece rook = (ChessPiece) board.removePiece(sourceR);
			board.placePiece(rook, targetR);
			rook.increaseMoveCount();
	...
	private void undeMove(Position source, Position target, Piece capturedPiece) {
		...
		// #specialmove en passant
		if (p instanceof Pawn) {
			if (source.getColum() != target.getColum() && capturedPiece == enPassantVulnerab) {
	...
	private void validateSourcePosition(Position position) {
		...
		if (!board.piece(position).isThereAnyPossibleMove()) {
			throw new ChessException("There is no possible moves for the chose.");
		}
